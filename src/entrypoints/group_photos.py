...
read_directory = Path(questionary.path("From which folder should photos be retrieved?").ask())
write_directory = Path(questionary.path("Where should the grouped photos be written to?").ask())
...
for (album_date, images) in images_grouped.items():
  ...
  if show_photos:
    ...
    max_width = int(questionary.text("What is the maximum pixel width of a window to show?", "1250").ask())
    max_height = int(questionary.text("What is the maximum pixel height of a window to show?", "750").ask())
  ...
  album_name = questionary.text("What description do you want to associate with the album?").ask()
